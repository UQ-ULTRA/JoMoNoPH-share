...
library(simsurv)
library(rstan)
# library(brms)  # Not used in this script
library(cmdstanr)
library(here)

# Set CmdStan path globally for main session
# set_cmdstan_path(here::here("..", ".cmdstan", "cmdstan-2.36.0"))  # Wrong version
# set_cmdstan_path("/Users/dingma/.cmdstan/cmdstan-2.36.0")
# Let cmdstanr auto-detect the installation (v2.37.0 found automatically)
...
joint_model_type <- "GB_aft"  # Change to "BP", "GB", "GB_Quantile", "GP", "BP_alt", "GB_aft", or "aft_only_BP_noRP" as needed
...
run_simulation <- function(i, n_patients, lambda_c, aft_mode, max_FU, unique_seed, joint_model_type = "BP", scenario = 2) {
  ...
  source(scenario_file)    
    ...
    if (joint_model_type == "BP") {
      ...
      stan_model_file_joint <- "Bernstein-Polynomials-alt.stan"
    } else if (joint_model_type == "GB_aft") {
      stan_model_file_joint <- "Gaussian-Basis.stan"
...
if (joint_model_type %in% c("aft_only_BP_noRP", "BP_alt", "GB_aft")) {
  # AFT-only models: only extract survival parameters
  joint_params <- do.call(rbind, lapply(results, function(r) {
    s <- r$joint_summary
    s[s$variable %in% c("beta_surv[1]", "beta_surv[2]"), 
      c("variable", "mean", "sd", "q5", "q95", "rhat")]
  }))
} else {
  # Joint models: extract both longitudinal and survival parameters
  joint_params <- do.call(rbind, lapply(results, function(r) {
    s <- r$joint_summary
